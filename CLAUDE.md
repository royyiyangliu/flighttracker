...
| 运营（Ops） | 沪京/沪深/沪穗/沪港航线机型、延误、准点率 | ceair.com + FlightView | `scripts/ops_daily.py` | `data/ops/` |
...
`ops_daily.py` 三步：
1. **发现航班号**：从 ceair.com 扫 20 条沪京/沪深/沪穗/沪港双向航线**明天**的 MU 航班号，**只收直达**（滤掉中转联程分段），合并进 `flights_list.json`（只增不减，随机顺序 + 8~15s 延迟防反爬）。列表里的 `arr` 仅为查询假设，真实航线以 FlightView 为准。
2. **抓运营数据**：用航班号去 FlightView 抓**昨天**的状态/机型/计划实际起降时刻/航站楼，算延误（含跨午夜修正），按 `(航班,日期,出发)` **upsert** 写入 `flights.csv`（整表重写、天然去重）。另**回补最近 3 天内到达为空且非取消的行**（航班延误跨天、抓取时尚未落地的情形）。4 并发、请求间隔 2s、屏蔽图片字体。
3. **生成前端 JSON**：按航线聚合 CSV（仅 20 条监控航线）→ 各 `{route}.json`。
...
- `cleanup_ops.py`：一次性脚本，按 CSV 真值重建 `flights_list.json`、清洗 `flights.csv`（删非监控航线行与重复副本）并重生成 JSON（默认 dry-run，`--apply` 才写入）。
