...
| 运营（Ops） | 沪京/沪深/沪穗航线机型、延误、准点率 | ceair.com + FlightView | `scripts/ops_daily.py` | `data/ops/` |
...
`ops_daily.py` 三步：
1. **发现航班号**：从 ceair.com 扫 16 条沪京/沪深/沪穗双向航线**明天**的 MU 航班号，**只收直达**（滤掉中转联程分段），合并进 `flights_list.json`（只增不减，随机顺序 + 8~15s 延迟防反爬）。列表里的 `arr` 仅为查询假设，真实航线以 FlightView 为准。
...
3. **生成前端 JSON**：按航线聚合 CSV（仅 16 条监控航线）→ 各 `{route}.json`。
